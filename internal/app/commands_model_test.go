...
func TestCmdModel_NoArgsDetectsClaudeCodeProviderImport(t *testing.T) {
	app := newModelCommandTestApp()
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	if got, want := len(ev.ModelBrowser.ImportSuggestions), 1; got != want {
		t.Fatalf("len(ev.ModelBrowser.ImportSuggestions) = %d, want %d", got, want)
	}
	suggestion := ev.ModelBrowser.ImportSuggestions[0]
	...
	for _, opt := range ev.ModelBrowser.Models.Options {
		switch opt.ID {
		case "__provider__kimi-for-coding":
			foundProvider = true
			if got := opt.DeleteProviderID; got != "" {
				t.Fatalf("env-backed provider DeleteProviderID = %q, want empty", got)
			}
		case "kimi-for-coding:k2p5":
			foundModel = true
		}
	}
	if !foundProvider {
		t.Fatal("expected env-backed provider row in model browser")
	}
	if !foundModel {
		t.Fatal("expected env-backed model option in model browser")
	}
}
